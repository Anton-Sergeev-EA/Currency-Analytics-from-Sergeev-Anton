...
Как это выглядит на реальных данных ЦБ РФ за 3 года: результат зависит от
валюты и от конкретного запуска обучения, и **некоторые валюты сейчас не
обгоняют наивный baseline** — это указывается открыто, в том же выводе
бэктеста, а не скрывается. Это реалистичный результат для модели,
обученной на публичных данных без доступа к закрытому потоку заявок или
высокочастотным сигналам, а не баг, который нужно тихо замаскировать
более красивой цифрой. При самостоятельном переобучении смотрите на
собственный вывод в консоли, а не на цифры в этом README — они устареют
уже к следующему переобучению модели.

Два признака были добавлены и затем сознательно убраны после того, как
реальный бэктест на живых данных показал, что с ними точность стала хуже,
а не лучше (признак "дней с последнего изменения ставки ЦБ" и признак
волатильности доходности увеличили ошибку у 3 из 4 валют) — решение
оставлено в виде комментария в коде и закреплённого теста в
`tests/test_ml.py`, а не тихо забыто: задокументированный отрицательный
результат тоже приносит пользу тому, кто прочитает код следующим.

Ансамбль также включает сам наивный прогноз ("завтра = сегодня") как
пятого кандидата без параметров, с весом по тому же правилу
кросс-валидации, что и у четырёх деревьев (см. докстринг
`EnsembleModel`). Это стандартный приём в прогнозировании — соревнования
вроде M4 регулярно выигрывают именно за счёт смешивания наивного /
статистического baseline с ML, а не противопоставления их друг другу —
а не способ выдать слабую модель за сильную: вес по-прежнему
определяется только измеренной точностью и может оказаться от почти
нулевого до доминирующего в зависимости от валюты и данных.
