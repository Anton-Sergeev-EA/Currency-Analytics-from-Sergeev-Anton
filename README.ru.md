...
## Точность модели — прочитайте перед тем, как доверять прогнозу

Курс валюты на день вперёд близок к случайному блужданию: значительная
часть завтрашнего движения курса действительно не предсказуема по его же
истории. Поэтому честный критерий "модель полезна" — это не высокий R²
сам по себе, а **обгон наивного baseline** ("завтрашний курс = сегодняшний
курс"), который для такого рода рядов уже сам по себе сильный и трудно
обгоняемый ориентир. Любой дашборд или продукт, который сообщает точность
прогноза валют без такого сравнения, не даёт никакой проверяемой
информации.

Этот проект показывает такое сравнение напрямую, считает его одинаково в
трёх местах: в консольном выводе `train_models.py` после каждого обучения,
в `/monitoring/api/model-accuracy` и в `GET /api/health` (поле
`components`). Все три используют один и тот же walk-forward бэктест
(`ModelEvaluator`): обучение строго на данных ДО отложенного окна,
прогноз на этом окне, модель никогда не видит его при обучении — и
результат усредняется по нескольким непересекающимся отложенным окнам,
а не только по самому последнему, чтобы одно удачное или неудачное окно
не переворачивало итоговый вердикт случайно.

**Честное текущее состояние на реальных данных ЦБ РФ за 3 года: ни одна
из четырёх валют пока не обгоняет наивный baseline полностью.** По
последнему реальному запуску обучения ближе всех GBP (RMSE бэктеста
0.865 против 0.855 у наивного прогноза — почти вплотную); USD, EUR и CNY
отстают заметнее. Это указывается открыто, в том же выводе бэктеста, а не
скрывается и не округляется в свою пользу. Это реалистичный результат для
модели, обученной на публичных данных без доступа к закрытому потоку
заявок или высокочастотным сигналам, а не баг, который нужно тихо
замаскировать более красивой цифрой. При самостоятельном переобучении
смотрите на собственный вывод в консоли, а не на цифры в этом README —
они устареют уже при следующем переобучении на другом окне истории.

Из честной борьбы с этим разрывом напрямую выросли два инженерных
решения — вместо того чтобы его скрыть:

- **Два признака были добавлены и затем сознательно убраны.** Признак
  "дней с последнего изменения ставки ЦБ" и признак волатильности
  доходности выглядели разумно, но реальный бэктест на живых данных
  показал, что они увеличили ошибку у 3 из 4 валют — решение оставлено в
  виде комментария в коде и закреплённого теста в `tests/test_ml.py`, а
  не тихо забыто.
- **Ансамбль теперь включает сам наивный прогноз** ("завтра = сегодня")
  как пятого кандидата без параметров, с весом по тому же правилу
  кросс-валидации, что и у четырёх деревьев (см. докстринг
  `EnsembleModel`) — стандартный приём в прогнозировании (смешивание
  наивного/статистического baseline с ML, как это делают
  побеждающие решения в соревнованиях вроде M4, а не противопоставление
  их друг другу). Это заметно сократило разрыв с наивным baseline у всех
  четырёх валют по сравнению с чистым ML, но не закрыло его полностью:
  наивный кандидат обычно получает 35–60% веса ансамбля, а остальной вес
  всё ещё уходит четырём ML-моделям, которые по отдельности отстают от
  baseline на этих данных — поэтому смесь лучше, чем чистый ML, но не
  гарантированно обгоняет чистую персистентность. Это не способ выдать
  слабую модель за сильную: вес по-прежнему определяется только
  измеренной точностью и применяется одинаково при обучении, в
  сохранённой боевой модели и в этом бэктесте.
...
| POST | `/api/refresh` | Фоновое обновление данных. **Требует заголовок** `X-Admin-Key: <SECRET_KEY>`. |
| POST | `/api/force-refresh` | Синхронное принудительное обновление данных, минуя кэш. **Требует заголовок** `X-Admin-Key: <SECRET_KEY>`. |
| GET | `/api/cache/status` | Статус кэша. **Требует заголовок** `X-Admin-Key: <SECRET_KEY>`. |
...
валюту: `train-{currency}` (число строк обучения, CV MAE и итоговый вес
каждого кандидата ансамбля, подобранные гиперпараметры) и
`backtest-{short_code}` (те же walk-forward `rmse`/`mae`/`mape`/`r2`, что
уже видны в консоли и на дашборде мониторинга, плюс сравнение с наивным
baseline). См. `src/infrastructure/ml/tracking.py`.

Сам сервер MLflow **не входит в этот репозиторий и не запускается через
Docker** — на продакшен-VDS он работает как отдельный systemd-сервис
(свой venv, своё sqlite-хранилище), независимо от всего, чем управляет
`docker-compose.yml`. `MLFLOW_TRACKING_URI` (по умолчанию
`http://172.17.0.1:5000` — шлюз Docker-моста, тем же способом, каким
контейнер уже достаёт Redis и Ollama на хосте) просто указывает на него.
Пустая строка полностью отключает логирование; пустой URI и недоступный
сервер обрабатываются одинаково — пропуск с предупреждением в лог, но
никогда не сбой обучения. Короткий HTTP-таймаут
(`MLFLOW_HTTP_REQUEST_TIMEOUT=5`, одна попытка повтора) не даёт
упавшему MLflow-серверу превратиться в многоминутное зависание.

**Разовая настройка на новом сервере:** у самого сервера MLflow есть
защита от DNS rebinding (`--allowed-hosts` в его systemd-юните) — он
принимает запросы только с распознанных заголовков `Host`. Адрес шлюза
Docker-моста туда по умолчанию не входит, добавь его один раз:
...
Без этого шага каждый ран честно логируется в консоль, но запись в
MLflow тихо не проходит с предупреждением `403 Invalid Host header` в
логах приложения — обучение всё равно завершается успешно, просто
дашборд MLflow остаётся пустым.

Проверено end-to-end на продакшн VDS (сентябрь 2026): после этого
исправления все 8 ранов (4 валюты x train + backtest) залогировались
успешно, с реальными URL-ами ранов, которые вывел сам клиент MLflow.

Чтобы посмотреть его вживую, пробросьте порт с VDS и откройте UI
локально (наружу, на публичном домене, он не торчит):

```bash
ssh -L 5001:127.0.0.1:5000 root@<ваш-ip-vds>
# затем откройте http://127.0.0.1:5001 в браузере
```

## Заметки по безопасности

Реальный аудит безопасности этого кода нашёл и исправил одну конкретную
проблему: `/api/refresh`, `/api/force-refresh` и `/api/cache/status` были
доступны любому в интернете без какой-либо проверки, а `/api/force-refresh`
запускает реальную работу (живой запрос к ЦБ РФ плюс полную перезагрузку
данных) без какого-либо ограничения частоты — тривиально уязвимо для
злоупотребления кем угодно, кто прочитал этот README. Теперь они требуют
заголовок `X-Admin-Key`, совпадающий с `SECRET_KEY` из `.env` (см.
`require_admin_key` в `src/presentation/api/routes/admin.py`). Если
`SECRET_KEY` всё ещё дефолтный плейсхолдер из `.env.example`, приложение
громко предупреждает об этом при старте — это значение публично лежит в
репозитории, так что проверка технически работает, но ничего не защищает,
пока не задать реальное значение.

Вторая реальная проблема была найдена и исправлена так же: `pip-audit
-r requirements.txt` нашёл известные CVE в `aiohttp` и, транзитивно, в
`starlette` — `fastapi` был закреплён на версии, чей диапазон зависимостей
ограничивал `starlette` версией ниже исправленной. Исправлено обновлением
`fastapi` до 0.141.1 (снимает это ограничение), прямым закреплением
`starlette` на 1.3.1 — минимальная версия, где закрыты все найденные
CVE — и обновлением `aiohttp`, `lxml`, `jinja2`, `python-dotenv` и
`lightgbm` до текущих стабильных релизов. Полный набор тестов (33 теста)
проходит на этих версиях, и `pip-audit -r requirements.txt` сейчас
не находит ни одной известной уязвимости. Со временем это снова
устареет — периодически перезапускай `pip-audit` и сверяй результат с
тестами перед дальнейшими обновлениями.

Одна вещь найдена, но сознательно **не** исправлена молча — неверное
исправление может быть хуже, чем задокументированный риск:

- **Контейнер сейчас работает от root** (`user: "0:0"` в
  `docker-compose.yml`), хотя `Dockerfile` создаёт и переключается на
  непривилегированного пользователя `analytics` — переопределение в
  compose почти наверняка стоит там из-за того, что примонтированные с
  хоста `./data` и `./logs` принадлежат root. Правильное исправление —
  сменить владельца этих каталогов на хосте (или подогнать UID/GID
  пользователя контейнера под них) — это согласованное изменение на
  реальном сервере, а не то, что можно переключить в одном файле, не
  проверив сначала, кому реально принадлежат эти пути. До тех пор
  компрометация процесса приложения даёт больше привилегий внутри
  контейнера, чем нужно.

Не найдено: в кодовой базе нет сырого SQL нигде (единственный доступ к БД
— лог A/B-теста — целиком идёт через ORM-запросы SQLAlchemy), и нет вызовов
