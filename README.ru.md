...
Две вещи найдены, но сознательно **не** исправлены молча — неверное
...
- **Версии зависимостей устаревают.** Периодически запускай
  `pip-audit -r requirements.txt` и сверяй результат с тестами проекта
  перед обновлением — часть пакетов (aiohttp, starlette через fastapi)
  обновляются не простым патчем.
