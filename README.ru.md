...
- Опциональный трекинг экспериментов в MLflow — обучение моделей и
  точность бэктеста (выключено по умолчанию, см. раздел «Трекинг
  экспериментов (MLflow)» ниже).
...
**Инфраструктура:** Docker, Docker Compose, Prometheus, Grafana, nginx (прод), MLflow (опционально, трекинг моделей).
...
## Трекинг экспериментов (MLflow, опционально)

Выключено по умолчанию (`USE_MLFLOW=False` в `.env`) — сервер MLflow не
обязателен для запуска приложения. Если сервер трекинга у вас есть
(например, `mlflow server --host 0.0.0.0 --port 5000`), укажите его в
конфиге:
...
Логируются два типа запусков, оба через отказоустойчивую обёртку
(`src/infrastructure/monitoring/mlflow_tracker.py`), которая никогда не
бросает исключение — если сервер трекинга недоступен, run просто
пропускается с предупреждением в лог, остальная работа приложения не
затрагивается:

- `train-{usd,eur}_rate` — логируется `train_models.py` при каждом
  переобучении: число строк, число признаков, вес каждой базовой модели
  в ансамбле, и сам файл `.joblib` как артефакт.
- `backtest-{usd,eur}` — логируется `/monitoring/api/model-accuracy`
  при каждом реальном пересчёте бэктеста на отложенной выборке (см.
  справочник API ниже; кешируется на час, то есть не чаще одного нового
  run в час на валюту): настоящие RMSE/MAE/MAPE/R² на тестовой выборке.
