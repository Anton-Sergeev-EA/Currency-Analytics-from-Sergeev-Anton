...
class FeatureEngineer:
    ...
    def create_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        #
        # "days_since_key_rate_change" и волатильность доходности
        # (std от pct_change) были добавлены и затем убраны в один и тот
        # же день: реальный walk-forward бэктест на VDS на живых данных
        # ЦБ (не синтетике) показал, что с ними точность стала хуже, а
        # не лучше - CNY перестал обгонять наивный прогноз (R2 0.69 ->
        # 0.6107), EUR/GBP тоже просели. На ~740 строках 2 лишних
        # разреженных/шумных признака дают модели больше шанса
        # переобучиться на шум, чем найти реальный сигнал. Решение
        # сохранено как явный факт истории (см. git log), а не тихо
        # забыто - при увеличении объёма обучающих данных эту гипотезу
        # стоит перепроверить, а не считать закрытой навсегда.
